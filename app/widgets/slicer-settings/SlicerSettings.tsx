...
export const SlicerSettings = () => {
  ...
  useState(() => {
    if (modelAnalysis && currentModel?.classification && suggestedSettings.length === 0) {
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className="flex items-center space-x-2">
                      <span className={`px-2 py-1 text-xs rounded ${
                        setting.quality === 'ultra-fine' ? 'bg-purple-100 text-purple-800' :
                        setting.quality === 'fine' ? 'bg-blue-100 text-blue-800' :
                        setting.quality === 'standard' ? 'bg-green-100 text-green-800' :
                        'bg-orange-100 text-orange-800'
                      }`}>
                        {setting.quality === 'ultra-fine' ? '최고품질' :
                         setting.quality === 'fine' ? '고품질' :
                         setting.quality === 'standard' ? '표준' : '드래프트'}
                      </span>
                      {printSettings?.id === setting.id && (
                  ...
                  <div className="grid grid-cols-3 gap-4 text-sm text-gray-600">
                    <div>
                      ...
                      <span className="ml-1 font-medium">{setting.layerHeight}mm</span>
                    </div>
                    <div>
                      <span className="text-gray-400">내부밀도:</span>
                      <span className="ml-1 font-medium">{setting.infillDensity}%</span>
                    </div>
                    <div>
                      <span className="text-gray-400">속도:</span>
                      <span className="ml-1 font-medium">{setting.printSpeed}mm/s</span>
                    </div>
                  </div>

                  <div className="mt-2 flex flex-wrap gap-1">
                    {setting.supportEnabled && (
                      <span className="px-2 py-1 bg-yellow-100 text-yellow-800 text-xs rounded">서포트</span>
                    )}
                    {setting.rafts && (
                      <span className="px-2 py-1 bg-blue-100 text-blue-800 text-xs rounded">래프트</span>
                    )}
                    {setting.brim && (
                      <span className="px-2 py-1 bg-green-100 text-green-800 text-xs rounded">브림</span>
                    )}
                  </div>
                </div>
              ))
          ...
          <div className="space-y-6">
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                ⚠️ 사용자 정의 설정은 고급 사용자를 위한 기능입니다. 
                잘못된 설정은 출력 실패나 품질 저하를 야기할 수 있습니다.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div>
                ...
                <select id="layerHeight" className="w-full border border-gray-300 rounded-md px-3 py-2">
                  <option value="0.1">0.1mm (최고품질)</option>
                  <option value="0.15">0.15mm (고품질)</option>
                  <option value="0.2" defaultValue="0.2">0.2mm (표준)</option>
              ...
              <div>
                <label htmlFor="infillDensity" className="block text-sm font-medium text-gray-700 mb-2">
                  내부 밀도 (%)
                </label>
                <input 
                  id="infillDensity"
              ...
              <div>
                <label htmlFor="wallCount" className="block text-sm font-medium text-gray-700 mb-2">
                  벽 개수
                </label>
                <input 
                  id="wallCount"
              ...
              <div>
                <label htmlFor="printSpeed" className="block text-sm font-medium text-gray-700 mb-2">
                  출력 속도 (mm/s)
                </label>
                <input 
                  id="printSpeed"
