...
export const useEstimation = () => {
  ...
  const createBambuLabSettings = (
    ...
    const quality =
      bambuLabSettings.bambuLabProfiles.qualityProfiles[qualityProfile];
    ...
    return {
      ...
      nozzleTemperature:
        filament.material.nozzleTemp.min +
        (filament.material.nozzleTemp.max - filament.material.nozzleTemp.min) /
          2,
      bedTemperature:
        filament.material.bedTemp.min +
  ...
  const calculateEstimationFromSettings = (
    settings: PrintSettings,
    modelVolume: number,
    selectedFilament: any
  ) => {
    const speedProfile = Object.values(
      bambuLabSettings.bambuLabProfiles.speedModes
    ...
    const baseTimePerLayer =
      (60 * settings.layerHeight) / (settings.printSpeed / 60); // 초/레이어
    ...
    const printTimeSeconds =
      layerCount * baseTimePerLayer * accelerationFactor * complexityFactor;
    ...
    // AMS 기반 정확한 비용 계산
    const filamentCost = amsManager.calculateFilamentCost(
      filamentWeightG,
      selectedFilament.material.type
    );
  ...
  const calculateEstimation = async (settings: PrintSettings) => {
    ...
    try {
      ...
      // 향상된 견적 계산
      const result = calculateEstimationFromSettings(
        settings,
        estimatedVolume,
        selectedFilament
      );
  ...
  const generateSuggestedSettings = (
  ...
  const formatFilamentUsage = (usage: {
    length: number;
    weight: number;
  }): string => {
