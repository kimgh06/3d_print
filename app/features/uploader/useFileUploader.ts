...
export const useFileUploader = () => {
  ...
  const validateFile = (file: File): boolean => {
    ...
    if (!extension || !allowedTypes.includes(extension)) {
      setError(
        "지원되지 않는 파일 형식입니다. STL, 3MF, OBJ 파일만 업로드 가능합니다."
      );
      return false;
    }
  ...
  const mockAnalyzeModel = async (
    ...
    return {
      ...
      estimatedVolume: Math.round(
        (fileSize / 1024) * 0.1 * formatComplexity + Math.random() * 100
      ),
      surfaceArea: Math.round(
        (fileSize / 1024) * 0.05 * formatComplexity + Math.random() * 50
      ),
      classification,
      fileFormat: fileType,
    };
  ...
  const uploadFile = async (file: File) => {
    ...
    try {
      ...
      // 추천 설정 생성
      const suggestedSettings = generateSuggestedSettings(
